...
```{r}
prediction <- predict(ourlogit, newdata = test)

regclass::confusion_matrix(ourlogit)
...
```{r}
set.seed(42)
our_rf <- randomForest(wine_type~flavanoids+alcohol+hue, data = train)


summary(our_rf)

prediction <- predict(our_rf, newdata = test)
prediction2 <- predict(our_rf, newdata = train)

trained <- data.frame(prediction2, train$wine_type)




tested <- data.frame(prediction, test$wine_type )

tested %>%
  mutate(same=if_else(prediction == test.wine_type, 1, 0)) %>%
  group_by(prediction, test.wine_type)%>%
  summarise(count=sum(same))
